...
#define COM_USE COM1			 // 个人定义，非库函数
#define RX_Buffer_USE RX1_Buffer // 个人定义，非库函数
	...
	if (UARTx == UART1)
	{
		...
		else if (COMx->UART_Mode == UART_ShiftRight)
		{
			if (COMx->BaudRateDouble == ENABLE)
				S1M0x6 = 1; // 固定波特率SysClk/2
			else
				S1M0x6 = 0; // 固定波特率SysClk/12
		}
		else if (COMx->UART_Mode == UART_9bit) // 固定波特率SysClk*2^SMOD/64
		{
			if (COMx->BaudRateDouble == ENABLE)
				SMOD = 1; // 固定波特率SysClk/32
			else
				SMOD = 0; // 固定波特率SysClk/64
		}
